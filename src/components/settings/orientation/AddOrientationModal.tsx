...
interface AddOrientationModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSaved: () => void;
}

export default function AddOrientationModal({ isOpen, onClose, onSaved }: AddOrientationModalProps) {
  ...
  const handleSubmit = async (values: OrientationInput) => {
    ...
    setError(null);
    try {
      await apiClient.post("/api/orientations", values);
