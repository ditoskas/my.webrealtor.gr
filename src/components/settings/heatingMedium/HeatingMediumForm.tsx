...
const EMPTY_VALUES: HeatingMediumInput = {
  name: "",
  slug: "",
};

interface HeatingMediumFormProps {
  initialValues?: Partial<HeatingMediumInput>;
  submitLabel: string;
  loading?: boolean;
  error?: string | null;
  onSubmit: (values: HeatingMediumInput) => void;
  onCancel: () => void;
}

export default function HeatingMediumForm({
  initialValues,
  submitLabel,
  loading = false,
  error,
  onSubmit,
  onCancel,
}: HeatingMediumFormProps) {
  const t = useTranslation();
  const [values, setValues] = useState<HeatingMediumInput>({ ...EMPTY_VALUES, ...initialValues });
