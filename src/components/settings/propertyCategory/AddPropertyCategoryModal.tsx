...
interface AddPropertyCategoryModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSaved: () => void;
}

export default function AddPropertyCategoryModal({ isOpen, onClose, onSaved }: AddPropertyCategoryModalProps) {
  ...
  const handleSubmit = async (values: PropertyCategoryInput) => {
    ...
    setError(null);
    try {
      await apiClient.post("/api/property-categories", values);
