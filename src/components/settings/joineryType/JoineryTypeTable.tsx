...
interface JoineryTypeTableProps {
  joineryTypes: JoineryType[];
  onEdit: (joineryType: JoineryType) => void;
  onDelete: (joineryType: JoineryType) => void;
}

export default function JoineryTypeTable({ joineryTypes, onEdit, onDelete }: JoineryTypeTableProps) {
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-neutral-100">
            {joineryTypes.length === 0 ? (
              ...
              joineryTypes.map((joineryType) => (
                <tr key={joineryType.id} className="hover:bg-neutral-50/60">
                  <td className="px-6 py-4 font-semibold text-neutral-900">{joineryType.name}</td>
                  <td className="px-6 py-4 text-xs text-neutral-400 font-mono">{joineryType.slug}</td>
