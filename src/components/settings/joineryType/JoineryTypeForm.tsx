...
const EMPTY_VALUES: JoineryTypeInput = {
  name: "",
  slug: "",
};

interface JoineryTypeFormProps {
  initialValues?: Partial<JoineryTypeInput>;
  submitLabel: string;
  loading?: boolean;
  error?: string | null;
  onSubmit: (values: JoineryTypeInput) => void;
  onCancel: () => void;
}

export default function JoineryTypeForm({
  initialValues,
  submitLabel,
  loading = false,
  error,
  onSubmit,
  onCancel,
}: JoineryTypeFormProps) {
  const t = useTranslation();
  const [values, setValues] = useState<JoineryTypeInput>({ ...EMPTY_VALUES, ...initialValues });
