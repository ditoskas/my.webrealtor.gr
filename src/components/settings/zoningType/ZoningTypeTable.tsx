...
interface ZoningTypeTableProps {
  zoningTypes: ZoningType[];
  onEdit: (zoningType: ZoningType) => void;
  onDelete: (zoningType: ZoningType) => void;
}

export default function ZoningTypeTable({ zoningTypes, onEdit, onDelete }: ZoningTypeTableProps) {
          ...
          <tbody className="divide-y divide-neutral-100">
            {zoningTypes.length === 0 ? (
              ...
              zoningTypes.map((zoningType) => (
                <tr key={zoningType.id} className="hover:bg-neutral-50/60">
                  <td className="px-6 py-4 font-semibold text-neutral-900">{zoningType.name}</td>
                  <td className="px-6 py-4 text-xs text-neutral-400 font-mono">{zoningType.slug}</td>
