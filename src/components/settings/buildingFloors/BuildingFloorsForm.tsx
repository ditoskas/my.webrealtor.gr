...
const EMPTY_VALUES: BuildingFloorsInput = {
  name: "",
  slug: "",
};

interface BuildingFloorsFormProps {
  initialValues?: Partial<BuildingFloorsInput>;
  submitLabel: string;
  loading?: boolean;
  error?: string | null;
  onSubmit: (values: BuildingFloorsInput) => void;
  onCancel: () => void;
}

export default function BuildingFloorsForm({
  initialValues,
  submitLabel,
  loading = false,
  error,
  onSubmit,
  onCancel,
}: BuildingFloorsFormProps) {
  const t = useTranslation();
  const [values, setValues] = useState<BuildingFloorsInput>({ ...EMPTY_VALUES, ...initialValues });
