...
const EMPTY_VALUES: FloorLevelInput = {
  name: "",
  slug: "",
};

interface FloorLevelFormProps {
  initialValues?: Partial<FloorLevelInput>;
  submitLabel: string;
  loading?: boolean;
  error?: string | null;
  onSubmit: (values: FloorLevelInput) => void;
  onCancel: () => void;
}

export default function FloorLevelForm({
  initialValues,
  submitLabel,
  loading = false,
  error,
  ...
  const [values, setValues] = useState<FloorLevelInput>({ ...EMPTY_VALUES, ...initialValues });
