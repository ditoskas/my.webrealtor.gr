...
const EMPTY_VALUES: RoadAccessTypeInput = {
  name: "",
  slug: "",
};

interface RoadAccessTypeFormProps {
  initialValues?: Partial<RoadAccessTypeInput>;
  submitLabel: string;
  loading?: boolean;
  error?: string | null;
  onSubmit: (values: RoadAccessTypeInput) => void;
  onCancel: () => void;
}

export default function RoadAccessTypeForm({
  initialValues,
  submitLabel,
  loading = false,
  error,
  onSubmit,
  onCancel,
}: RoadAccessTypeFormProps) {
  const t = useTranslation();
  const [values, setValues] = useState<RoadAccessTypeInput>({ ...EMPTY_VALUES, ...initialValues });
