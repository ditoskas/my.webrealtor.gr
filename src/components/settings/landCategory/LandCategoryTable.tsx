...
interface LandCategoryTableProps {
  ...
  onEdit: (landCategory: LandCategory) => void;
  onDelete: (landCategory: LandCategory) => void;
}

export default function LandCategoryTable({ landCategories, onEdit, onDelete }: LandCategoryTableProps) {
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-neutral-100">
            {landCategories.length === 0 ? (
              ...
              landCategories.map((landCategory) => (
                <tr key={landCategory.id} className="hover:bg-neutral-50/60">
                  <td className="px-6 py-4 font-semibold text-neutral-900">{landCategory.name}</td>
                  <td className="px-6 py-4 text-xs text-neutral-400 font-mono">{landCategory.slug}</td>
