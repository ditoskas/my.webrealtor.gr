...
const EMPTY_VALUES: LandCategoryInput = {
  name: "",
  slug: "",
};

interface LandCategoryFormProps {
  initialValues?: Partial<LandCategoryInput>;
  submitLabel: string;
  loading?: boolean;
  error?: string | null;
  onSubmit: (values: LandCategoryInput) => void;
  onCancel: () => void;
}

export default function LandCategoryForm({
  initialValues,
  submitLabel,
  loading = false,
  error,
  onSubmit,
  onCancel,
}: LandCategoryFormProps) {
  const t = useTranslation();
  const [values, setValues] = useState<LandCategoryInput>({ ...EMPTY_VALUES, ...initialValues });
