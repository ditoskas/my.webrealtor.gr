...
interface GlassTypeTableProps {
  ...
  onEdit: (glassType: GlassType) => void;
  onDelete: (glassType: GlassType) => void;
}

export default function GlassTypeTable({ glassTypes, onEdit, onDelete }: GlassTypeTableProps) {
          ...
          <tbody className="divide-y divide-neutral-100">
            {glassTypes.length === 0 ? (
              ...
              glassTypes.map((glassType) => (
                <tr key={glassType.id} className="hover:bg-neutral-50/60">
                  <td className="px-6 py-4 font-semibold text-neutral-900">{glassType.name}</td>
                  <td className="px-6 py-4 text-xs text-neutral-400 font-mono">{glassType.slug}</td>
