...
interface HeatingSystemTableProps {
  heatingSystems: HeatingSystem[];
  onEdit: (heatingSystem: HeatingSystem) => void;
  onDelete: (heatingSystem: HeatingSystem) => void;
}

export default function HeatingSystemTable({ heatingSystems, onEdit, onDelete }: HeatingSystemTableProps) {
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-neutral-100">
            {heatingSystems.length === 0 ? (
              ...
              heatingSystems.map((heatingSystem) => (
                <tr key={heatingSystem.id} className="hover:bg-neutral-50/60">
                  <td className="px-6 py-4 font-semibold text-neutral-900">{heatingSystem.name}</td>
                  <td className="px-6 py-4 text-xs text-neutral-400 font-mono">{heatingSystem.slug}</td>
                  <td className="px-6 py-4 text-right space-x-2">
                    ...
                    <Button
                      ...
                      onClick={() => onDelete(heatingSystem)}
                    >
                      <Trash2 size={14} />
                    </Button>
                  </td>
