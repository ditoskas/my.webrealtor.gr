...
interface SlopeTableProps {
  slopes: Slope[];
  onEdit: (slope: Slope) => void;
  onDelete: (slope: Slope) => void;
}

export default function SlopeTable({ slopes, onEdit, onDelete }: SlopeTableProps) {
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-neutral-100">
            {slopes.length === 0 ? (
              ...
              slopes.map((slope) => (
                <tr key={slope.id} className="hover:bg-neutral-50/60">
                  <td className="px-6 py-4 font-semibold text-neutral-900">{slope.name}</td>
                  <td className="px-6 py-4 text-xs text-neutral-400 font-mono">{slope.slug}</td>
