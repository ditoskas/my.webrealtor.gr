...
const EMPTY_VALUES: SlopeInput = {
  name: "",
  slug: "",
};

interface SlopeFormProps {
  initialValues?: Partial<SlopeInput>;
  submitLabel: string;
  loading?: boolean;
  error?: string | null;
  onSubmit: (values: SlopeInput) => void;
  onCancel: () => void;
}
...
export default function SlopeForm({
  ...
  const [values, setValues] = useState<SlopeInput>({ ...EMPTY_VALUES, ...initialValues });
