...
// Not a route — a colocated helper shared by POST (route.ts) and PUT ([id]/route.ts), same
// reasoning as app/api/properties/parsePropertyBody.ts (shared coercion instead of hand-mapping
// the same ~30 fields twice). Next's router only treats route.ts/page.tsx (etc.) as routable, so
...
// Images are managed on the dedicated media page (POST /api/uploads + PUT /api/lands/[id]/images),
...
export interface ParsedLandBody {
  errors: string[];
  data: Partial<ILand>;
}

export function parseLandBody(body: Record<string, unknown>): ParsedLandBody {
  const errors: string[] = [];
  ...
  const data: Partial<ILand> = {
    ...
    landCategoryId: toObjectId(body.landCategoryId),
