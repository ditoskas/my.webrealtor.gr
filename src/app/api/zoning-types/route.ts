...
export async function GET() {
  try {
    const data = await ZoningTypeService.list();
    return NextResponse.json({ data, success: true });
  } catch (error) {
    console.error("GET /api/zoning-types error", error);
...
export async function POST(request: Request) {
  try {
    ...
    const existing = await ZoningTypeService.findBySlug(slug);
    if (existing) {
      return NextResponse.json({ message: "A zoning type with this slug already exists" }, { status: 409 });
    }

    const data = await ZoningTypeService.create({ name, slug });

    await LogEntryService.info({
      category: "ZoningType",
      message: `Zoning type "${data.name}" (${data.slug}) was created`,
    ...
    console.error("POST /api/zoning-types error", error);
    return NextResponse.json({ message: "Internal server error" }, { status: 500 });
  }
}
