...
export async function GET() {
  try {
    const data = await BuildingFloorsService.list();
    return NextResponse.json({ data, success: true });
  } catch (error) {
    console.error("GET /api/building-floors error", error);
...
export async function POST(request: Request) {
  try {
    ...
    const existing = await BuildingFloorsService.findBySlug(slug);
    if (existing) {
      return NextResponse.json({ message: "A building floors with this slug already exists" }, { status: 409 });
    }

    const data = await BuildingFloorsService.create({ name, slug });

    await LogEntryService.info({
      category: "BuildingFloors",
      message: `Building floors "${data.name}" (${data.slug}) was created`,
    ...
    console.error("POST /api/building-floors error", error);
    return NextResponse.json({ message: "Internal server error" }, { status: 500 });
  }
}
